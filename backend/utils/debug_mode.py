...
PARSER_COLORS = {name: light_blue_1 for name in ['parse_xml']}
PERSON_COLORS = {f'Person.{name}': light_red_2 for name in ['__init__', 'add_conversation']}
CONVO_COLORS = {f'Conversation.{name}': light_orange_1 for name in ['__init__', 'add_message']}
MESSAGE_COLORS = {f'Message.{name}': yellow for name in ['__init__', 'set_timestamp']}

COLORS = {}
COLORS.update(PARSER_COLORS)
COLORS.update(PERSON_COLORS)
COLORS.update(CONVO_COLORS)
COLORS.update(MESSAGE_COLORS)
