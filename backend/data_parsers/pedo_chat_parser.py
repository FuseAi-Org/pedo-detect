...
def parse_xml_and_save(xml_file: str):
    ...
    for person in people:
        filename = f'././unzipped-data/parsed-pedo-chat-data/people/{person.id}.json'
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        save_object_to_json(person.to_dict(), filename)
    # Save Conversation objects
    for convo in convos:
        filename = f'.././unzipped-data/parsed-pedo-chat-data/conversations/{convo.id}.json'
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        save_object_to_json(convo.to_dict(), filename)
    people_files = os.listdir('././unzipped-data/parsed-pedo-chat-data/people')
    convo_files = os.listdir('././unzipped-data/parsed-pedo-chat-data/conversations')
...
def parse_xml_and_save_optimized(xml_file: str, should_save:bool = False) -> None:
    ...
    for conv in root.findall('conversation'):
        ...
        if not convo_exists:
            ...
            if should_save:
                filename = f'././unzipped-data/parsed-pedo-chat-data/conversations/{conversation.id}.json'
                os.makedirs(os.path.dirname(filename), exist_ok=True)
    ...
    for person_id in people_dict:
        ...
        if should_save:
            filename = f'././unzipped-data/parsed-pedo-chat-data/people/{person_id}.json'
            os.makedirs(os.path.dirname(filename), exist_ok=True)
...
parse_xml_and_save_optimized('././unzipped-data/raw-pedo-chat-data/Training-Data/pan12-sexual-predator-identification-training-corpus-2012-05-01.xml')
